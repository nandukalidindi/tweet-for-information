...
class YoutubeQueryWorker
    ...
    if user_keyword.present?
      ...
      begin
        ...
        search_response.items.each_with_index do |search_result, index|
          ...
          when 'youtube#video'
            user_keyword.user_keyword_hits.create(provider: 'youtube', uri: "https://www.youtube.com/watch?v=#{search_result.id.video_d}", content: search_result.snippet.title, score: (((len-index).to_f/len.to_f) * user_keyword.relevance * 0.9))
          end
        end
      rescue Exception => e
  ...
  def get_service
   client = Google::Apis::YoutubeV3::YouTubeService
